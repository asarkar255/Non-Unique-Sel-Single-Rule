...
# NEW: render ORDER BY list using catalog PK (alias-qualified if available)
...
# ---------- generator in requested layout (ORDER BY last) ----------
def build_remediated_select(full: str,
    ...
    Emit:
      ...
      [ORDER BY <pk>].
      ENDSELECT.
    Always places ORDER BY (if keys known) as the last clause before the period.
    Existing ORDER BY in source is ignored/omitted.
    """

    # WHERE slice (verbatim 'WHERE ...' without trailing period)
    ...
    # INTO slice (verbatim, cut at next clause)
    into_line = ""
    ...
    # ORDER BY (computed from PK)
    ob = render_order_by_keys(table, alias)
    order_by_line = f"ORDER BY {ob}" if ob else ""

    # Header
    head_lines = [
        f"SELECT {fields.strip()}",
    ...
    # put the terminating period on ORDER BY, else on WHERE, else on UP TO
    if order_by_line:
        out_lines.append(order_by_line + ".")
    elif where_line:
...
def analyze_unit(u: Unit) -> Dict[str, Any]:
    ...
    for m in SELECT_BLOCK_RE.finditer(code):
        ...
        message_parts.append("Rewritten to classic SELECT ... ENDSELECT with UP TO 1 ROWS and ORDER BY <primary_key> at the end.")
